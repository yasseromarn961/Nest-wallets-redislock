...
export class AdminService {
  ...
  async listAdmins(
    ...
    try {
      const result: unknown = await this.adminAuthServiceClient.listAdmins(
        token,
        query,
      );
      return result;
    } catch (error: any) {
      this.logWinston('error', 'admin_list_error', {
  ...
  async getAdminById(token: string, id: string): Promise<unknown> {
    try {
      const result: unknown = await this.adminAuthServiceClient.getAdminById(
        token,
        id,
      );
      return result;
    } catch (error: any) {
      this.logWinston('error', 'admin_get_by_id_error', {
        id,
        error: normalizeError(error),
      });
      throw toHttpExceptionFromExternal(error, this.i18n);
